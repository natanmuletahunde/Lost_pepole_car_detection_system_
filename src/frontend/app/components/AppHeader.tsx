...
// Reusable Navigation Header Component (can be used across all pages)
"use client";
...
export default function AppHeader({
      ...
      const fetchUnread = async () => {
        try {
          const res = await apiClient(
            `${process.env.NEXT_PUBLIC_API_BASE_URL || "http://localhost:5000/api/v1"}/notifications`,
          );
          if (res.ok) {
  ...
  const unreadCount =
    propUnreadCount !== undefined ? propUnreadCount : internalUnread;
  const colorScheme = propColorScheme || (mantineColorScheme as any) || "light";

  const toggleColorScheme =
    propToggleColorScheme ||
    (() => {
  ...
  const isReportedCases = pathname.startsWith("/user/reported-cases");

  const showGoToDashboard =
    propShowGoToDashboard !== undefined
    ...
    textNodes.forEach((textNode) => {
      ...
      while ((match = regex.exec(textVal)) !== null) {
        ...
        if (matchIndex > lastIndex) {
          frag.appendChild(
            document.createTextNode(textVal.substring(lastIndex, matchIndex)),
          );
        }
        ...
        <Group justify="space-between" wrap="nowrap">
          ...
          <Group gap={isMobile ? "xs" : "sm"} wrap="nowrap">
            ...
            <Tooltip
              ...
              offset={12}
            >
              <ActionIcon className="lpc-nav-btn" onClick={toggleColorScheme}>
                {colorScheme === "dark" ? (
                  <IconSun size={22} />
                ) : (
                  <IconMoon size={22} />
